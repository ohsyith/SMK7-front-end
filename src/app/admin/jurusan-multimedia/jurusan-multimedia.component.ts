...
@Component({
  selector: 'app-jurusan-multimedia',
  templateUrl: './jurusan-multimedia.component.html',
  styleUrls: ['./jurusan-multimedia.component.css']
})
export class JurusanMultimediaComponent  implements OnInit{
  singkatan: string = 'DKV'; 
  ...
  saveChanges(): void {
    ...
    this.dataService.loadKompetensiKeahlian('DKV').subscribe((data: any) => {
      ...
      this.dataService.editJurusan('DKV', formData).subscribe({
        next: (_) => {
...
this.filteredData = this.MULTIMEDIA.filter((data) => {
...
getDataIduka() {
  this.dataService.loadKompetensiKeahlian(this.jurusan).subscribe((data: any) => {
    this.MULTIMEDIA = data.kerjaSama.map((item: any, index: number) => {
      return {
        NO: index + 1,
    ...
    this.MULTIMEDIA = this.MULTIMEDIA.filter((item: any) => item.Kerjasama !== null && item.Kerjasama !== undefined);
    this.performSearch();
  });
}

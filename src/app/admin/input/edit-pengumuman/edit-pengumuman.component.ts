...
@Component({
  selector: 'app-edit-pengumuman',
  templateUrl: './edit-pengumuman.component.html',
  styleUrls: ['./edit-pengumuman.component.css']
})
export class EditPengumumanComponent {
  ...
  onFileChange(event: any) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      this.dataService.imageberita(this.DataBerita.judul, formData)
        .subscribe({
					next: (_) => {
						this.notif.success('Success :', 'Berhasil Menambahkan Foto Pengumuman ');
						this.getData();
						this.isMain();
						},
					error: (_) => this.notif.error('Galat :', 'Terjadi Kesalahan Saat Menambahkan Foto Pengumuman !'),
				});
    }
  ...
  editSubmit(){
    ...
    this.dataService.editBerita(this.idUrl, formData).subscribe({
      next: (_) => {
        this.notif.success('Success :', 'Berhasil Memperbaharui Pengumuman Sekolah');
        this.getData();
        this.router.navigateByUrl('/admin/Pengumuman')
        
        },
      error: (_) => this.notif.error('Galat :', 'Terjadi Kesalahan Saat Memperbaharui Pengumuman Sekolah !'),
    })
  }

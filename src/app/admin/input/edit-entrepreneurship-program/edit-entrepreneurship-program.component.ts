...
@Component({
  selector: 'app-edit-entrepreneurship-program',
  templateUrl: './edit-entrepreneurship-program.component.html',
  styleUrls: ['./edit-entrepreneurship-program.component.css']
})
export class EditEntrepreneurshipProgramComponent implements OnInit {
  selectedImage: any;
  judul: string = "";
  ...
  loadData() {
    ...
    if (id) {
      this.dataService.getHubunganId(+id).subscribe(data => {
  ...
  saveChangeSambutan() {
    ...
    this.dataService.updateHubungan(+id, formData).subscribe({
      next: (_) => {this.notif.success('Success :', 'Berhasil Memperbaharui Daftar Hubungan Industri'),
this.router.navigateByUrl("/admin/Hubungan-Industri")
      },
      error: (_) => this.notif.error('Galat :', 'Terjadi Kesalahan Saat Memperbaharui Daftar Hubungan Industri !'),
    });    
  }

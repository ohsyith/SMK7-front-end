...
@Component({
  selector: 'app-jurusan-teknik-komputer-dan-jaringan',
  templateUrl: './jurusan-teknik-komputer-dan-jaringan.component.html',
  styleUrls: ['./jurusan-teknik-komputer-dan-jaringan.component.css']
})
export class JurusanTeknikKomputerDanJaringanComponent implements OnInit{
  ...
  saveChanges(): void {
    ...
    this.dataService.loadKompetensiKeahlian('TKJ').subscribe((data: any) => {
      ...
      this.dataService.editJurusan('TKJ', formData).subscribe(
        {
					next: (_) => {
...
this.filteredData = this.tkj.filter((data) => {
...
getDataIduka() {
  this.dataService.loadKompetensiKeahlian(this.jurusan).subscribe((data: any) => {
    this.tkj = data.kerjaSama.map((item: any, index: number) => {
      return {
        NO: index + 1,
    ...
    this.tkj = this.tkj.filter((item: any) => item.Kerjasama !== null && item.Kerjasama !== undefined);
    this.performSearch();
  });
}

...
@Component({
  selector: 'app-jurusan-teknik-jaringan-akses-telkomunikasi',
  templateUrl: './jurusan-teknik-jaringan-akses-telkomunikasi.component.html',
  styleUrls: ['./jurusan-teknik-jaringan-akses-telkomunikasi.component.css']
})
export class JurusanTeknikJaringanAksesTelkomunikasiComponent implements OnInit{
  singkatan: string = 'tjat'; 
  ...
  saveChanges(): void {
    ...
    this.dataService.loadKompetensiKeahlian('tjat').subscribe((data: any) => {
      ...
      this.dataService.editJurusan('tjat', formData).subscribe(
        {
					next: (_) => {
...
this.filteredData = this.tjat.filter((data) => {
...
getDataIduka() {
  this.dataService.loadKompetensiKeahlian(this.jurusan).subscribe((data: any) => {
    this.tjat = data.kerjaSama.map((item: any, index: number) => {
      return {
        NO: index + 1,
    ...
    this.tjat = this.tjat.filter((item: any) => item.Kerjasama !== null && item.Kerjasama !== undefined);
    this.performSearch();
  });
}

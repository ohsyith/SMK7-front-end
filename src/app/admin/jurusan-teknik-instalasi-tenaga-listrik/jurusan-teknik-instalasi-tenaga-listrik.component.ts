...
@Component({
  selector: 'app-jurusan-teknik-instalasi-tenaga-listrik',
  templateUrl: './jurusan-teknik-instalasi-tenaga-listrik.component.html',
  styleUrls: ['./jurusan-teknik-instalasi-tenaga-listrik.component.css']
})
export class JurusanTeknikInstalasiTenagaListrikComponent implements OnInit {
  singkatan: string = 'titl'; 
  ...
  saveChanges(): void {
    ...
    this.dataService.loadKompetensiKeahlian('titl').subscribe((data: any) => {
      ...
      this.dataService.editJurusan('titl', formData).subscribe(
        {
					next: (_) => {
...
this.filteredData = this.titl.filter((data) => {
...
getDataIduka() {
  this.dataService.loadKompetensiKeahlian(this.jurusan).subscribe((data: any) => {
    this.titl = data.kerjaSama.map((item: any, index: number) => {
      return {
        NO: index + 1,
    ...
    this.titl = this.titl.filter((item: any) => item.Kerjasama !== null && item.Kerjasama !== undefined);
    this.performSearch();
  });
}
